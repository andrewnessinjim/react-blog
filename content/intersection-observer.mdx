...
This post assumes famaliarity with the following topics:
...
Imagine a web page as the box with a dashed outline as below. Within it, we have an element represented by a balloon.
...
<Quiz>
  ...
  >
    <Choice>😍 appears when I stop observing.</Choice>
    <Choice correct={true}>😍 appears when I start observing.</Choice>
    <Choice correct={true}>
      When the status is observing, 😍 appears once when the balloon enters the
      viewport.
    </Choice>
    <Choice>
      When the status is not observing, 😍 appears once when the balloon enters
      the viewport.
    </Choice>
    <Choice correct={true}>
      When the status is observing, 😍 appears once when the balloon exits the
      viewport.
    </Choice>
  </MultipleChoiceQuestion>
</Quiz>
...
This `callback` will be invoked by the `observer` every time something interesting happens. Our callback simply logs "Interesting". In the interactive demo, this event was represented by the emoji 😍 appearing.
...
This example only focuses on the Intersection Observer API. In the real-world, we also have to handle error states and ensure we don't make network requests repeatedly in spite of errors. This can be done by hiding the observed element in error states. We can also avoid funky behaviors by hiding the observed element during the loading state.

## Conclusion

I hope this helped you get a hold on how the API works! Head on to [the official docs][1] to dig deeper. The API supports specifying multiple threshold levels, knowing the exact intersection ratio and much more! You can also find a list of use cases for this API at the beginning of the page; the infinite scrolling example was one of them.
