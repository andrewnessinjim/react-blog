...
The `callback` will be invoked by the `observer` when something _interesting_ happens. Our callback simply logs "Interesting". In the interactive demo, this callback invocation is represented by the emoji 😍 appearing.
...
How does the `observer` know what is interesting? We have two knobs to tell the observer what we are interested in. The first one is to specify the target (I'll come back to the second knob shortly):
...
Here, the target a zero-height element that is positioned at the end of the viewport:
